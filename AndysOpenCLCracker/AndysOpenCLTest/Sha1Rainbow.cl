 ...
 */
__kernel void sha1Rainbow(__global uint* out, __global uchar* in, __constant int* passLen)
{
	//Local Workgroup Mem Instance Kernel Variables:
	uint temp, A, B, C,	D, E;
    uint W[16];
	//__local uint passwordLen;
	//passwordLen = *passLen;

	//SM Private Register Mem Instance Variables:
	//__local uchar inputMem[passwordLen];
	...
	//__private int offset = get_global_offset(0);
	//__private int multiplier = get_global_offset(1);
	...
	{
		paddedMessageBlock[i] = in[inputIndexStart + i];// inputMem[i];//wavefrontInput[(localId * passwordLen) + i];
	}
	...
	out[outputIndexStart + 0] = intermediateHashStorage[0];
